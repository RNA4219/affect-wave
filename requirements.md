...
参照実装は、Anthropic 論文の感情概念空間に可能な限り近づけるため、**内部層として 171 概念相当の fine-grained emotion concept bank** を持たなければならない。

ここでいう 171 概念相当とは、論文の概念空間を参考にした細粒度概念群であり、名称や定義は論文の概念空間に可能な限り近づけるべきだが、PoC では厳密同一性までは要求しない。

内部 fine-grained concept bank は、少なくとも以下を満たさなければならない。

* concept ごとの prototype embedding を持つこと

* concept ごとの textual definition を持つこと

* canonical 8 emotion labels への写像表を持つこと

* valence / arousal / appraisal への集約規則を持つこと

* 概念総数は参照実装で **171 前後を目標** とし、少なくとも docs 上で concept coverage を追跡可能にすること
...
canonical label set は **表示層・公開 I/F 用の集約ラベル** として扱い、内部推定の最小単位にしてはならない。



* 内部 fine-grained concept score を concept ごとの prototype embedding との類似度から算出すること

* `top_emotions` は、fine-grained concept score を canonical 8 labels へ集約した後の順位として算出すること

* `valence` / `arousal` は、あらかじめ定義した軸 prototype との相対類似度または fine-grained concept score の重み付き合成から導出されること

* `appraisal` は、appraisal ごとの prototype との類似度または fine-grained concept score の固定重み写像から導出されること

* 推定器が embedding を取得せずに LLM のテキストだけで値を決めてはならないこと

* 類似度計算、重み、prototype 定義、concept-to-canonical mapping の所在を docs またはコード内で追跡可能にしなければならないこと
...
* `top_emotions` は internal fine-grained concept score を canonical 8 labels へ集約した結果でなければならない
...
### 11.4.1 fine-grained concept 要件

* 参照実装では、内部細粒度概念スコア群 `concept_scores` を保持しなければならない

* `concept_scores` は concept id, canonical label, score を少なくとも持たなければならない

* 通常 UI に 171 概念相当の生配列を露出してはならない

* デバッグ出力では、171 概念相当の `concept_scores` 全件を数値で確認できなければならない

* `params mode` では全件露出を必須としないが、必要に応じて preview を返してよい
...
* 参照実装では、`density` と `jitter` は fine-grained concept score の分散・競合度・不安定度を反映する方向で補正しなければならない
...
出力モードは **params mode** と **wave mode** を必須とする。

### 14.1 params mode

params mode はデフォルト出力でなければならない。

少なくとも以下を満たさなければならない。

* JSON 形式で返すこと

* `top_emotions`、`trend`、`compact_state`、`wave_parameter` を明示できること

* 外部エージェントがそのまま利用できること
...
params mode は外部エージェント連携と内部数値確認の主経路であり、以下を満たさなければならない。
...
* params mode は通常の UX と API 連携の既定値として扱ってよい

* params mode はデバッグ・解析・研究用途としても扱える
...
Discord adapter は、少なくとも以下の 2 系統の操作方式を持たなければならない。

* slash command

* メッセージ内トリガー

slash command は英語ベースの固定コマンド面を正本とし、参照実装では少なくとも以下を持たなければならない。

* `/affect wave`

* `/affect params`

* `/affect transport reply_prefix`

* `/affect transport webhook`

メッセージ内トリガーは補助導線として扱い、日本語と英語の双方を受け付けなければならない。

* 日本語トリガー例: `詳細`, `感情波`, `パラメータ`

* 英語トリガー例: `detail`, `wave`, `params`

参照実装では、英語のみ、または日本語のみの片対応にしてはならない。

#### params mode トリガー

Discord 上で params mode を呼び出す明示的操作は、少なくとも以下のいずれかで成立しなければならない。

* slash command `/affect params`

* 日本語トリガーを含む補助メッセージ

* 英語トリガーを含む補助メッセージ

通常会話のみで自動的に params mode を露出してはならない。

#### 失敗時挙動

Discord adapter は、表示用 transport が失敗した場合でも通常応答本文を優先しなければならない。

* `reply_prefix` 失敗時は本文のみ返してよい

* `webhook` 失敗時は `reply_prefix` へ degrade してよい

* params mode の取得失敗は wave mode の本文送信を阻害してはならない

#### 複数 turn 追従

Discord 上で 3 turn 以上連続した場合でも、以下を維持しなければならない。

* wave 表現の表示位置がぶれない

* 同一 turn の wave mode と params mode の対応が追跡できる

* transport 切替後も同一 turn の wave parameter を再利用できる
...
* slash command とメッセージ内トリガーの少なくとも 1 つずつで params mode を呼び出せること

* 日本語トリガーと英語トリガーの双方で詳細表示を呼び出せること

* 同一 turn の wave mode と params mode が同じ wave parameter を参照していること

* 連続 3 turn 以上で表示崩れなく追従すること

* webhook 失敗時に通常応答が失われないこと
...
* 171 概念相当の fine-grained concept bank と canonical 8 labels への写像が定義されている
...
* `top_emotions` が fine-grained concept score の集約結果として算出されている

* デフォルトで `params mode` 表示を返せる

* 補助的に `wave mode` 表示を返せる
...
* API で動作させる初期公開版では hidden state にアクセスできないため、本版は埋め込み経由の擬似推定として実装するべきである

* 論文により近い hidden state / activation ベースのローカル版は、API版とは別ラインで設計・実装すべきである
...
**API LLM と `llama.cpp` 上の軽量 multilingual 埋め込みモデルを組み合わせ、推定した affect state を wave parameter に変換し、デフォルトでは `top_emotions`・`trend`・`wave_parameter` を返す params mode を主表示とし、補助的に擬音・AA による wave mode を併設する affect expression interface を実装し、感情制御ではなく情動表現の可視化を行う。**
